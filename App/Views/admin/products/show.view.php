        ...
        <table class="min-w-full bg-white rounded-xl text-sm">
            ...
            <tbody>
                <?php if (empty($products)): ?>
                    <tr>
                        <td colspan="5" class="px-4 py-4 text-center text-blue-400">No products found.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($products as $product): ?>
                        <tr class="border-t border-blue-100 hover:bg-blue-50 transition">
                            <td class="px-4 py-2 whitespace-nowrap">
                                <div class="font-semibold text-blue-900"><?= e($product->productname) ?></div>
                                <div class="text-blue-500 text-xs"><?= e($product->email) ?></div>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap text-blue-700"><?= e($product->role) ?></td>
                            <td class="px-4 py-2 whitespace-nowrap flex items-center space-x-2">
                                <a href="<?= url('admin/products/' . $product->id) ?>" class="bg-blue-100 text-blue-700 hover:bg-blue-200 border border-blue-300 rounded px-2 py-1 flex items-center gap-1 transition shadow-sm" title="View">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4 w-4">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.75a5.25 5.25 0 1 1 0 10.5 5.25 5.25 0 0 1 0-10.5Zm0 0v3m0-3h3m-3 3H9m9.75-6H2.25A2.25 2.25 0 0 0 0 6v12a2.25 2.25 0 0 0 2.25 2.25h19.5A2.25 2.25 0 0 0 24 18V6a2.25 2.25 0 0 0-2.25-2.25Z" />
                                    </svg>
                                </a>
                                ...
                                <form action="<?= url('admin/products/' . $product->id . '/delete') ?>" method="POST" style="display:inline;">
                                    <?= csrf_field() ?>
                                    <button type="submit" class="bg-red-100 text-red-700 hover:bg-red-200 border border-red-200 rounded px-2 py-1 flex items-center gap-1 transition shadow-sm" title="Delete" onclick="return confirm('Delete this product?')">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4 w-4">
