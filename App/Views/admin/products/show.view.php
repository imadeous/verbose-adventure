                ...
                <div class="flex flex-col items-center mt-4 gap-2">
                    <?php if (!empty($gallery)): ?>
                        <div class="relative w-64 h-40">
                            <?php foreach ($gallery as $idx => $image): ?>
                                <img
                                    ...
                                    alt="<?= e($product->name) ?>"
                                    class="absolute left-0 top-0 w-64 h-40 object-cover rounded-lg shadow-md border border-blue-100 transition-all duration-300"
                                    style="z-index:<?= count($gallery) - $idx ?>; opacity:<?= $idx === 0 ? '1' : '0.5' ?>; transform: translateY(<?= $idx * 8 ?>px) scale(<?= 1 - $idx * 0.05 ?>);"
                                    id="carousel-image-<?= $idx ?>"
                                    <?= $idx !== 0 ? 'hidden' : '' ?> />
                            <?php endforeach; ?>
                            <?php if (count($gallery) > 1): ?>
                                <button type="button" class="absolute left-2 top-1/2 -translate-y-1/2 bg-blue-100 text-blue-700 rounded-full p-1 shadow hover:bg-blue-200 transition" onclick="carouselPrev()">
                                    &#8592;
                                </button>
                                <button type="button" class="absolute right-2 top-1/2 -translate-y-1/2 bg-blue-100 text-blue-700 rounded-full p-1 shadow hover:bg-blue-200 transition" onclick="carouselNext()">
                                    &#8594;
                                </button>
                            <?php endif; ?>
                        </div>
                        <div class="flex gap-2 mt-2">
                            <?php foreach ($gallery as $idx => $image): ?>
                                <button type="button" class="w-3 h-3 rounded-full <?= $idx === 0 ? 'bg-blue-600' : 'bg-blue-200' ?>" onclick="carouselGoTo(<?= $idx ?>)"></button>
                            <?php endforeach; ?>
                        </div>
                        <script>
                            let currentIdx = 0;
                            const totalImages = <?= count($gallery) ?>;
                            ...
                            function showImage(idx) {
                                ...
                                document.querySelectorAll('.w-3.h-3.rounded-full').forEach((dot, i) => {
                                    dot.className = 'w-3 h-3 rounded-full ' + (i === idx ? 'bg-blue-600' : 'bg-blue-200');
                                });
                            }
